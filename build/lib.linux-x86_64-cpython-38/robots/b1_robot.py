...
class B1Robot(b1.B1):
  """Interface for real B1 robot."""
  MPC_BODY_MASS = 50
  l_x = 1.108
  l_y = 0.671
  l_z = 0.461
  I_x = (1/12)*MPC_BODY_MASS*(l_y**2+l_z**2)
  ...
  ACTION_CONFIG = [
      ...
      locomotion_gym_config.ScalarField(name="RR_thigh_joint",
                                        upper_bound=4.18879020479,
                                        lower_bound=-1.0471975512),
      locomotion_gym_config.ScalarField(name="RR_cald_joint",
                                        upper_bound=-0.916297857297,
                                        lower_bound=-2.69653369433),
  ...
  def __init__(self, pybullet_client, time_step=0.002, **kwargs):
    ...
    kwargs['on_rack'] = True
    super(B1Robot, self).__init__(pybullet_client,
                                  time_step=time_step,
                                  **kwargs)
  ...
  def ApplyAction(self, motor_commands, motor_control_mode=None):
    ...
    if motor_control_mode == robot_config.MotorControlMode.POSITION:
      ...
      raise ValueError('Unknown motor control mode for B1 robot: {}.'.format(
          motor_control_mode))
  ...
  def Reset(self, reload_urdf=True, default_motor_angles=None, reset_time=3.0):
    ...
    super(B1Robot, self).Reset(reload_urdf=reload_urdf,
    ...
    if not default_motor_angles:
      default_motor_angles = b1.INIT_MOTOR_ANGLES
