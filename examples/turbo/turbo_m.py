...
class TurboM(Turbo1):
    ...
    def optimize(self, seed_str):
        """Run the full optimization process."""
        ig         = config()
        filename   = ig.data_samples_dir + name_turbo() + 'seed_' + seed_str  + '.p'
#        dict_input = load_dictionary('/data/alireza/high_bo/BayesOpt/tests/results/turbo_training_points/' + name_turbo() + 'seed_'+seed_str +'.p')
        '''
        arr = os.listdir(ig.data_samples_dir)
        for i in arr:
            if 'turbo' in i and 'init' in i:

                dict_input = load_dictionary(ig.data_samples_dir+i)
                break

        for i in range(self.n_trust_regions):
            X_init        = dict_input['Xepisodes'][i*self.n_init:(i+1)*self.n_init].detach().numpy()
            self.X        = np.vstack((self.X, X_init))
            self.fX       = np.vstack((self.fX, np.array([[self.f(x)] for x in X_init]) ))
        ...
        for i in range(self.n_trust_regions):
            if i != 0:
               ...
               if self.initialization != 'no':
                  self.max_evals = 200
                  X_init = latin_hypercube(self.n_init, self.dim)
                  X_init = from_unit_cube(X_init, self.lb, self.ub)
               else:
                  X_init = self.f.gait_params.init_X.reshape(1,-1)
                  X_init = np.concatenate([X_init] * self.n_init, axis=0)
#               X_init = np.vstack((X_init, self.f.gait_params.init_X.reshape(1,-1)))
        ...
        while self.n_evals < self.max_evals:
            ...
            if self.n_evals%10 == 0:
               dict_out = OrderedDict([('Xepisodes', self.X),('Yepisodes', self.fX)])
