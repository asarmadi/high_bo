...
_MPC_WEIGHTS = (0.5, 0.5, 0.02, 0, 0, 1, 0.05, 0.05, 0.02, 0.02, 0.02, 0.01, 0)
_U_WEIGHTS   = (1e-5,)*12
...
class TorqueStanceLegController(leg_controller.LegController):
  ...
  def __init__(
      ...
      mpc_weights: Sequence[float] = _MPC_WEIGHTS,
...
#    mpc_weights[12]             = 0.0
    weights_list = list(mpc_weights[0:-12])
    u_list = list(mpc_weights[-12:])

#    weights_list = list(_MPC_WEIGHTS)
#    u_list = list(_U_WEIGHTS)
  ...
  def get_action(self):
    ...
    predicted_contact_forces = self._cpp_mpc.compute_contact_forces(
        [0],  #com_position
        np.asarray(self._state_estimator.com_velocity_body_frame, dtype=np.float64),  #com_velocity
    ...
    for i in range(self._num_legs):
      contact_forces[i] = np.array(predicted_contact_forces[i * _FORCE_DIMENSION:(i + 1) * _FORCE_DIMENSION])
    action = {}
