...
public:
    ...
    ConvexMpc(double mass, const std::vector<double>& inertia, int num_legs,
        ...
        const std::vector<double>& qp_weights, const std::vector<double>& u_weights, 
...
ConvexMpc::ConvexMpc(double mass, const std::vector<double>& inertia,
    ...
    alpha_(AsBlockDiagonalMat(u_weights, planning_horizon)),
    alpha_single_(AsBlockDiagonalMat(u_weights, 1)),
//    alpha_(alpha* MatrixXd::Identity(num_legs* planning_horizon* k3Dim, num_legs* planning_horizon* k3Dim)),
//    alpha_single_(alpha* MatrixXd::Identity(num_legs* k3Dim, num_legs* k3Dim)),
    ...
    p_mat_(num_legs* planning_horizon* k3Dim, num_legs* planning_horizon* k3Dim),
    q_vec_(num_legs* planning_horizon* k3Dim),
    anb_aux_(kStateDim* planning_horizon, action_dim_),
    constraint_(kConstraintDim* num_legs* planning_horizon, action_dim_* planning_horizon),
...
PYBIND11_MODULE(mpc_osqp, m) {
  ...
  py::class_<ConvexMpc>(m, "ConvexMpc")
      .def(py::init<double, const std::vector<double>&, int,
          int , double ,const std::vector<double>&, const std::vector<double>&, QPSolverName>())
