...
from tfbo.utils.load_save import load_keras_model, save_dictionary
from tfbo.utils.name_file import name_model_vae, name_synthetic
...
class vae_bo_optimizer(optimizer):
    ...
    def run(self, dict_args, maxiters):
        ...
        filename = path_dict + name_synthetic(dict_args) + 'seed_' + str(dict_args['seed'][0]) + '.p'

        path = '/data/alireza/high_bo/BayesOpt/Baselines/chemvae/' + \
               self.dict_args['obj'] + '/'
        encoder, decoder = self.load_vae_models(path)
        ...
        for i in range(maxiters):
            ...
            if i%10==0:
               dict_out = OrderedDict(
                  [
                    ('Xepisodes', self.data_x[None]),
                    ('Yepisodes', self.data_y)
                  ])
               save_dictionary(filename, dict_out)

        return self.data_x[None], self.data_y
