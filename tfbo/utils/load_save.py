...
def loadMat(filename):
    path = '/data/alireza/high_bo/BayesOpt/datasets/tasks/'
    try:
...
def load_partname(path, partname):
    path_to_os = '/data/alireza/high_bo/BayesOpt'
    try:
        os.chdir(path_to_os)
...
# load start
def load_initializations(dict_args, names, seed_l=None):
    ...
    if seed_l != None:
       x_l, y_l = np.empty((1,0,88)), np.empty((0,1))
       for seed_i in seed_l:
           dict_start['seed'] = seed_i
           x_file, y_file = name_attr(dict_start)
           y_start = attr_partload(path=path, partname=y_file)
           x_start = attr_partload(path=path, partname=x_file)
           x_l = np.append(x_l, x_start, axis=1)
           y_l = np.append(y_l, y_start, axis=0)
       return [(x_l, y_l)]
    else:
...
def load_SRinitializations(dict_args, names):
    ...
    full_path = '/data/alireza/high_bo/BayesOpt/' + path
    def load_pair(seed_i):
        dict_start['seed'] = seed_i
        x_file, y_file = name_attr(dict_start)
